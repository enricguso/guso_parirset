...
if __name__ == "__main__":
    ...
    parser.add_argument("--treverberant", dest='treverberant', action='store_true')    
    parser.add_argument("--tnoisy", dest='tnoisy', action='store_true')    
    ...
    if not args.tnoisy and not args.treverberant:
        ...
        model_name = 'M1_clean_e03'
        with open('/home/enric.guso@local.eurecat.org/guso_parirset/conf/e3_lr0004_s2.yaml', 'r') as file:
            config = ConfigDict(yaml.load(file, Loader=yaml.FullLoader))
    elif not args.tnoisy and args.treverberant:
        print("Evaluating reverberant version of the model")
        model_name = 'M2_reverberant_d3'
        with open('/home/enric.guso@local.eurecat.org/guso_parirset/conf/parirset_conf.yaml', 'r') as file:
            config = ConfigDict(yaml.load(file, Loader=yaml.FullLoader))
    elif args.tnoisy and not args.treverberant:
        print("Evaluating noisy version of the model")
        model_name = 'M3_noisy_crowdrylrE2'
        with open('/home/enric.guso@local.eurecat.org/guso_parirset/conf/crowdioset_dry_lrE2.yaml', 'r') as file:
            config = ConfigDict(yaml.load(file, Loader=yaml.FullLoader))
    else: 
        print("Evaluating noisy and reverberant version of the model")
        model_name = 'M4_noisyrev_crowwetlrE2'
    ...
    if args.tnoisy:
        seperator = Seperator2(model, os.path.join(model_path,'model.th'))
    else:
    ...
    for i, song in tqdm.tqdm(enumerate(songs)):
        ...
        with torch.no_grad():
            ...
            #for models trained without audience, set the audience estimate to 0 for evaluation
            if args.tnoisy:
                ...
                sdrs = new_sdr(sources, 
                            torch.stack((torch.from_numpy(estimates['drums'].T), 
                                            ...
                                            torch.from_numpy(estimates['other'].T),
                                            torch.from_numpy(estimates['vocals'].T),
                                            torch.zeros_like(torch.from_numpy(estimates['drums'].T)))).unsqueeze(0))
    ...
    df.to_excel(os.path.join('/media/diskA/enric/parirset_models/results_noisyrev/' + model_name + '.xlsx'), index=False)